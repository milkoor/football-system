...
CREATE TABLE IF NOT EXISTS match_records (
    ...
    is_completed        INTEGER NOT NULL DEFAULT 0,  -- 是否完成: 0=未完成, 1=已完成
    match_id            TEXT,                      -- 关联系统A的match_id
    created_at          TEXT NOT NULL DEFAULT (datetime('now')),
    updated_at          TEXT NOT NULL DEFAULT (datetime('now'))
);
...
class ConfigStore:
    ...
    def create_league(
        ...
        # 增强鲁棒性：验证联赛名字不为空
        if not name_zh or name_zh.strip() == "":
            # 如果名字为空，生成一个默认名字
            import time
            default_name = f"未命名联赛_{int(time.time())}"
            logger.warning(f"创建联赛时名字为空，使用默认名字: {default_name}")
            name_zh = default_name
    ...
    def upsert_match_records(
        ...
        try:
            ...
            cursor = self._conn.execute(
                "SELECT id, round, home_team, away_team, is_completed, match_id "
                "FROM match_records "
                "WHERE season_instance_id = ? AND play_type = ? AND timing = ?",
                (season_instance_id, play_type, timing),
            )
            for row in cursor.fetchall():
                # 使用 (round, home_team, away_team) 作為唯一標識
            ...
            processed_count = 0
            for r in records:
                key = (r.round_num, r.home_team, r.away_team)

                if key in existing_records:
                    # 記錄已存在
                    existing_row = existing_records[key]

                    if existing_row["is_completed"]:
                        # 已完成的比賽，跳過更新
                        logger.info(
                            f"跳過已完成比賽: {r.home_team} vs {r.away_team} (Round {r.round_num})"
                        )
                        continue
                    else:
                        # 未完成的比賽，更新
                        self._conn.execute(
                            ...
                            SET
                                score = ?, x_value = ?, sim_result = ?, link = ?,
                                settlement_value = ?, settlement_direction = ?,
                                home_away_direction = ?, target_team = ?,
                                match_id = ?, updated_at = datetime('now'),
                                is_completed = ?
                            WHERE id = ?
                            """,
                            (
                                r.score, r.x_value, r.settlement, r.link,
                                r.settlement_value, r.settlement_direction,
                                r.home_away_direction, r.target_team,
                                r.match_id, r.is_completed,
                                existing_row["id"]
                            ),
                        )
                        logger.info(
                            f"更新未完成比賽: {r.home_team} vs {r.away_team} (Round {r.round_num})"
                        )
                else:
                    # 新記錄，插入
                    self._conn.execute(
                        ...
                        "is_completed, match_id, updated_at) "
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))",
                        (
                            season_instance_id,
                            play_type,
                    ...
                    logger.info(
                        f"插入新比賽: {r.home_team} vs {r.away_team} (Round {r.round_num})"
                    )
    ...
    def _row_to_match_record(row: sqlite3.Row) -> MatchRecord:
        return MatchRecord(
            ...
            is_completed=bool(row["is_completed"] if "is_completed" in row else 0),
            match_id=row["match_id"] if "match_id" in row else "",
        )
