...
> 文档版本: 1.0 / 适用版本: v1.1.0+
> **v1.2.0 更新说明**: X 值计算跳过逻辑已从基于 `score_ft` 正则匹配优化为基于赔率 `elapsed_time >= 85` 判断（更准确，避免"推迟"/"取消"等文字比分误判）。`data_connector.py` 中的 X 值计算现已直接调用本地 `XValueCalculator.calculate_from_odds_data()`，预取赔率数据避免二次请求。
